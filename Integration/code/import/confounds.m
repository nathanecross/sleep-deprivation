...
%% Sleep
subs = subject.SubjectList;
times = [{'ses-02','desc','one_scan'} 
         {'ses-02','run-01','scan_1'}
         {'ses-02','run-02', 'scan_2'}];
     
% Load confounds for tasks
for t = 1:3   
    [Subjectdata,NumberOfSubjects,SubjectName,noFuncFolder] = load_confounds(sub_dir,times(t,1),times(t,2),char(times(t,3)),'task-nap',format);
    Subjectnap.(char(times(t,3))) = Subjectdata.(char(times(t,3)));
end
...
for z = 1:NumberOfSubjects
    for t = 1:length(times)
        if isfield(Subjectnap.one_scan,(char(SubjectName{z})))
            FDnap.nap.(char(SubjectName{z})) = Subjectnap.one_scan.(char(SubjectName{z})).framewise_displacement;
        else
           FDnap.nap.(char(SubjectName{z})) = [Subjectnap.scan_1.(char(SubjectName{z})).framewise_displacement ...
                                            Subjectnap.scan_2.(char(SubjectName{z})).framewise_displacement];                            
        end
    end
    FDnap.mean(z,1) = mean(FDnap.nap.(char(SubjectName{z})));
end
