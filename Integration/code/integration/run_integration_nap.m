...
for z = 1:NumberOfSubjects
    ...
    elseif z==20
        ...
        load( [sub_dir sprintf('%s/%s/%s_nap.mat',  ...
            (char(SubjectName{z})),'nap_trunc',(char(SubjectName{z})))] );
        fmri{z,1} = Parcels;
    end
end
...
Itot = zeros(NumberOfSubjects,1); I_inter = zeros(NumberOfSubjects,1); 
I_intra = zeros(NumberOfSubjects,NumberOfNets,1);

NumberOfSubjects = 20; zz=0;
for z = 1:20
    if z == 9
        Itot(z,1) = NaN;
        I_inter(z,1) = NaN;
    ...
    elseif z == 20
        ...
        zz = zz+1;
        Itot(z,1) = HI.Nap{zz, 1}.int_total.mean;
        I_inter(z,1) = HI.Nap{zz, 1}.int_inter.mean;
        for n = 1:NumberOfNets
            I_intra(z,n,1) = HI.Nap{zz, 1}.int_intra.mean(n,n);
        end
        
        FCR(z,1) = sum(I_intra(z,:,1))/HI.Nap{zz, 1}.int_inter.mean;
    end
end
Iws = sum(I_intra,2);
HI.Itot = Itot; HI.Iws = Iws; HI.Ibs = I_inter; HI.FCR = FCR;
