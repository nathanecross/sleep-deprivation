...
for t = 1:length(times(:,1))
    for z= 1:NumberOfSubjects 
        ...
        load([ sub_dir char(SubjectName{z}) '/' char(times(t,3)) '/' char(SubjectName{z}) '_' char(times(t,2)) '_' keyword '.mat'])
...
for z= 1:NumberOfSubjects 
    fprintf(['Correlating timeseries between parcels for NREM2 - ' char(SubjectName{z}) '\n']);
    filelist = struct2table(dir([ sub_dir char(SubjectName{z}) '/nap_trunc']));
    filelist = filelist.name;
    if length(filelist)> 0
        file = char(filelist(contains(filelist,keyword)));
        if length(file)> 0
            load([ sub_dir char(SubjectName{z}) '/nap_trunc/' file])

            ConMat.corr =  corr(Parcels');
            fprintf( ['Performing Fisher r to z transformations for NREM2 - ' char(SubjectName{z}) '\n' ]);
            ConMat.corr_z = atanh(ConMat.corr);
            ConMat.corr_z(ConMat.corr_z>=8)=NaN;
            if ~exist(sprintf('%s/',out_dir), 'dir')
                mkdir(sprintf('%s/',out_dir))
            end
            if ~exist(sprintf('%s/nap_trunc/%s',out_dir,char(SubjectName{z})), 'dir')
                mkdir(sprintf('%s/nap_trunc/%s',out_dir,char(SubjectName{z})))
            end
            if ~exist(sprintf('%s/%s/nap_trunc/',out_dir,char(SubjectName{z})), 'dir')
                mkdir(sprintf('%s/%s/nap_trunc/',out_dir,char(SubjectName{z})))
            end
            % And save the data
            save(sprintf('%s/%s/nap_trunc/%s_%s.mat',out_dir,char(SubjectName{z}),char(SubjectName{z}),keyword),'ConMat');
        end
    end
end
